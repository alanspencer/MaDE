...
// then clears `nexusParserset'.
NexusParserSetReader::NexusParserSetReader(NexusParserToken &t, int maxValue, QMap<int, int> &ism, NexusParserBlock &b, int type) : token(t), block(b), intSetMap(ism)
{
    max = maxValue;
    settype = type;
    intSetMap.clear();
    ...
    for (;;)
    {
        ...
        if (token.equals("-")){
            ...
            if (insideRange){
                ...
                throw NexusParserException(block.errorMessage,
            ...
            modValue = NexusParserToken::demandPositiveInt(token, block.errorMessage, "The modulus value");
...
// are stored in `nexusParserset', however, with offset 0. For example, if the data file says "4-10\2" this function would be
...
bool NexusParserSetReader::addRange(int first, int last, int modulus)
{
...
// which it then converts to a number. Failing that, it throws a NexusParserException exception.
int NexusParserSetReader::getTokenValue()
{
    int v = token.getToken().toInt();

    if (v == 0 && settype != NexusParserSetReader::generic){
        if (settype == NexusParserSetReader::charset){
            v = block.charLabelToNumber(token.getToken());
        } else if (settype == NexusParserSetReader::taxset) {
